...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const postFarm = (origin, target, templateId) =>
    new Promise((resolve, reject) => {
      TribalWars.post(
        ...
        null,
        { target, template_id: templateId, source: origin },
        (r) => resolve(r),
        (err) => reject(err)
      );
    });
            ...
            .on('click', () => {
              ...
              getData(
                ...
                if (game_data.market === 'nl') {
                  const firstOrigin = $('.farmGod_icon')
                    .first()
                    .data('origin');
                  if (firstOrigin) {
                    const $btn = $(
                      `.switchVillage[data-id="${firstOrigin}"]`
                    ).first();
  ...
  const buildTable = function (plan) {
    let html = `<div class="vis farmGodContent"><h4>FarmGod</h4>
                <div style="display:flex;gap:8px;justify-content:center;margin:6px 0;">
                  <input type="button" class="btn farmGod_startAuto" value="Start auto" />
                  <input type="button" class="btn farmGod_stopAuto" value="Stop" />
                </div>
  ...
  const ensureVillageFor = async (originId) => {
    if (game_data.market !== 'nl') return true;
    if (curVillage === originId) return true;

    const $btn = $(
      `.switchVillage[data-id="${originId}"]`
    ).first();
    if ($btn.length) {
      $btn.trigger('click');
  ...
  const pickNextIcon = () => $('.farmGod_icon').first();
  ...
  const runAuto = async (opts = {}) => {
    ...
    try {
      while (!autoStopRequested) {
        ...
        const originId = +$icon.data('origin');
        const canSend = await ensureVillageFor(originId);
        if (!canSend) {
          $icon.closest('.farmRow').remove();
          continue;
        }

        await sendFarm($icon, opts);
  ...
  const sendFarm = async function ($icon, opts = {}) {
    const {
      minDelayMs = 350,
      maxDelayMs = 900,
      maxRetries = 2,
    ...
    if (farmBusy) return { skipped: true, reason: 'busy' };
    if (
      Accountmanager.farm.last_click &&
      now - Accountmanager.farm.last_click < minDelayMs
    ) {
    ...
    const origin = +$icon.data('origin');
    const target = +$icon.data('target');
    const templateId = +$icon.data('template');
    ...
    try {
      ...
      while (true) {
        try {
          const r = await postFarm(origin, target, templateId);
          ...
          if (attempt > maxRetries) {
            UI.ErrorMessage(e || t.messages.sendError);
            $icon.closest('.farmRow').remove();
            return { ok: false, error: e };
          }

          await sleep(
            retryBaseDelayMs * attempt + jitter(200, 700)
          );
        }
      }
    } finally {
